...
export abstract class Opt<T> {
   ...
   */
    propOrNull< //
    K extends (T extends object ? keyof T : never), //
    R extends (T extends object ? WithoutOptValues<T[K]> | null : never) //
    >(key: K): R {
    return this.prop(key).orNull() as R;
  }
  ...
  propOrUndef< //
    K extends (T extends object ? keyof T : never), //
    R extends (T extends object ? WithoutOptValues<T[K]> | undefined : never) //
    >(key: K): R {
    return this.prop(key).orUndef() as R;
  }
  ...
  propOrZero< //
    K extends (T extends object ? keyof T : never), //
    R extends (T extends object ? WithoutOptValues<T[K]> | 0 : never) //
    >(key: K): R {
...
class None<T> extends Opt<T> {
  ...
  flatMapIn<U, R>(this: None<U[]>, _f: (x: U) => R[]): Opt<R[]> { return none; }
...
class Some<T> extends Opt<T> {
  ...
  flatMapIn<U, R>(this: Some<U[]>, f: (x: U) => R[]): Opt<R[]> {
    if (!isArray(this._value)) { throw new Error('flatMapIn called on non array: ' + this._value); }
    return some(this._value.flatMap(f));
  }
...
export const flatMapIn = <T, U>(f: (a: T) => U[]) => (x: Opt<T[]>): Opt<U[]> => x.flatMapIn(f);
...
export const isEmpty = (x: PossiblyEmpty): boolean => {
  ...
  if (isObject(x)) return Object.getOwnPropertyNames(x).length === 0;
  if (isString(x)) return x === '';
  if (isNumber(x)) return Number.isNaN(x);
